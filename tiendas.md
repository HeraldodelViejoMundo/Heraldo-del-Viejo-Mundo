...
<div class="tn-mission">
  <p>
    <strong>El Heraldo del Viejo Mundo</strong> nace con la voluntad de ser <em>punto de encuentro y herramienta de cabecera</em> para cuantos servimos en las filas del Viejo Mundo bajo la lengua de Cervantes: jugadores veteranos que recuerdan las viejas batallas, pintores que dan vida a las miniaturas, generales noveles que se inician en estas tierras y todo aquel que, en suma, comparta la afición por <em>Warhammer&nbsp;Fantasy</em> y <em>Warhammer: The Old World</em>.
  </p>
  <p>
    En ese empeño se enmarca este <strong>Mapa de los Mercaderes</strong>: un directorio cuidadosamente reunido de las tiendas que sostienen el hobby a lo largo y ancho de la geografía hispana —desde los reinos del norte hasta las marcas del sur—, para que ningún general quede sin saber dónde proveer su ejército ni en qué mesa lo desplegará.
  </p>
</div>

<div class="tn-disclaimer">
  Última actualización: <strong>abril de 2026</strong>. Los datos pueden variar con el tiempo; conviene confirmar horarios y dirección antes de desplazarse.<br>
  ¿Falta alguna tienda o hay algún dato incorrecto? Háznoslo saber escribiendo a <a href="mailto:heraldo@elheraldodelviejomundo.com" style="color:#8b5a2b;font-weight:600;">heraldo@elheraldodelviejomundo.com</a> y lo corregiremos en la siguiente revisión.
</div>
...
(function(){
  ...
  function cardHtml(s) {
    ...
    if (s.email) {
      const emails = s.email.split(/\s*[·,]\s*/).filter(e => e.trim());
      const links = emails.map(e => '<a href="mailto:' + escapeHtml(e) + '">' + escapeHtml(e) + '</a>');
      meta.push('✉ ' + links.join(' · '));
    }
